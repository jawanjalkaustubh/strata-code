#!/usr/bin/env bash
# =============================================================================
#  Strata Code - macOS SHORTCUTS (the twin of Install-Shortcuts.ps1)
#
#  Builds a small launcher app bundle and puts it where a Mac keeps shortcuts:
#    ~/Applications/Strata Code.app   (Launchpad, Spotlight, drag to the Dock)
#    ~/Desktop/Strata Code            (alias of the same app)
#  The bundle carries the app icon and runs run-strata-code.command from this checkout,
...
# no Terminal window, and keeps the launcher's output in ~/Library/Logs/StrataCode/launch.log. On a first
...
if [ -d "\$REPO/node_modules/electron/dist/Electron.app" ] && [ -f "\$REPO/dist-electron/main.js" ]; then
  LOGDIR="\$HOME/Library/Logs/StrataCode"; mkdir -p "\$LOGDIR"
